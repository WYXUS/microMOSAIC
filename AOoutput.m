...
AOtask.Timing.ConfigureSampleClock('',100000, SampleClockActiveEdge.Rising, SampleQuantityMode.FiniteSamples)    % a clock

AOtask.Control(TaskAction.Verify)   % control the task

AOwriter = AnalogSingleChannelWriter(AOtask.Stream);     % create a writer
%%
AOwriter.WriteMultiSample(true,linspace(-10,10,1e3)');   % write voltages to a galvo
%%
AOtask.Stop()
AOtask.Dispose()
%%
